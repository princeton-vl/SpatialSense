...
def load_features(opts):
    ...
    data = defaultdict(lambda : {'raw_spatial_feature': [], 'discretized_spatial_feature': [], 
                                 'appearance_feature': [], 'predicate': [], 'label': [], '_id': []})
    dataset = json.load(open(opts.data_path))
    bar = ProgressBar(max_value=len(dataset))
    for i, img in enumerate(dataset):
        for annot in img['annotations']:
             raw_fea = raw_spatial_feature(annot['subject']['bbox'], annot['object']['bbox']).astype(np.float32)
             discretized_fea = gmm.predict_proba(raw_fea.reshape(1, -1)).squeeze().astype(np.float32)
             predicate_onehot = np.zeros(len(predicate_categories), dtype=np.float32)
             predicate_onehot[predicate_categories.index(annot['predicate'])] = 1.
             ...
             if opts.appr:
                 appr_fea = pca.transform(np.vstack(raw_appearance_feature(img['url'], annot['subject']['bbox'], annot['object']['bbox'])))
    ...
    if opts.no_val:
        data['train']['raw_spatial_feature'].extend(data['valid']['raw_spatial_feature'])
        data['train']['discretized_spatial_feature'].extend(data['valid']['discretized_spatial_feature'])
        ...
        if opts.appr:
            data['train']['appearance_feature'].extend(data['valid']['appearance_feature'])

    for split in data.keys():
        indexes = list(range(len(data[split]['raw_spatial_feature'])))
        random.shuffle(indexes)
        print('%d samples in %s' % (len(indexes), split))
        data[split]['raw_spatial_feature'] = torch.Tensor(np.vstack(data[split]['raw_spatial_feature'])[indexes]).cuda()
        data[split]['discretized_spatial_feature'] = torch.Tensor(np.vstack(data[split]['discretized_spatial_feature'])[indexes]).cuda()
        data[split]['predicate'] = torch.Tensor(np.vstack(data[split]['predicate'])[indexes]).byte().cuda()
        data[split]['label'] = torch.Tensor([1. if label == True else -1. for label in data[split]['label']])[indexes].cuda()
        data[split]['_id'] = list(np.asarray(data[split]['_id'])[indexes])
        if opts.appr:
            data[split]['appearance_feature'] = torch.Tensor(np.vstack(data[split]['appearance_feature'])[indexes]).cuda()
    return data
...
if __name__ == '__main__':
    ...
    optimizer = torch.optim.RMSprop(model.parameters(), lr=opts.learning_rate, weight_decay=opts.l2)
    if opts.no_val:
    ...
    for n_iter in range(opts.num_iters):
        logits = model(data['train']['discretized_spatial_feature'] if opts.spatial else None, 
        ...
        if n_iter % opts.val_interval == (opts.val_interval - 1):
            if opts.no_val:
                ...
                model.eval()
                logits = model(data['valid']['discretized_spatial_feature'] if opts.spatial else None,
                               data['valid']['appearance_feature'] if opts.appr else None,
                               data['valid']['predicate'])
                loss = criterion(logits, data['valid']['label'])
                print('validation loss = %f' % loss)
                correct = (logits > 0) * (data['valid']['label'] > 0) + (logits <= 0) * (data['valid']['label'] <= 0)
    ...
    print('testing..')
    model.eval()
    logits = model(data['test']['discretized_spatial_feature'] if opts.spatial else None,
                   data['test']['appearance_feature'] if opts.appr else None,
                   data['test']['predicate'])
    correct = (logits > 0) * (data['test']['label'] > 0) + (logits <= 0) * (data['test']['label'] <= 0)
    for i, predicate in enumerate(predicate_categories):
